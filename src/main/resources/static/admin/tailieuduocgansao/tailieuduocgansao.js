app.controller("tailieuduocgansao-ctrl", function ($scope, $http,Tag) {
    $scope.favfile = [];
    $scope.favfolder = [];
    ...
    $scope.fileall = function () {
        ...
        $http.get("/rest/favorites/filefavorite").then(resp => {
            $scope.favfile = resp.data;
        });
        $http.get("/rest/favorites/folderfavorite").then(resp => {
            $scope.favfolder = resp.data;
    ...
    $scope.pager = {
        ...
        get fav() {
            ...
            return $scope.fav.slice(start, start + this.size);
        },
        get count() {
            return Math.ceil(1.0 * $scope.fav.length / this.size);
        },
        first() {
            this.page = 0;
        },
...
// Phân quyền hiẻn thị
    ...
    $scope.menu = "Tài liệu được gắn sao"
    $scope.quanly = "tailieuduocgansao"
    $scope.fileall();
});
(function () {
    'use strict';
