app.controller("giaoviectudong-ctrl", function ($scope, $http, Tag) {
    $scope.folder = [];
    $scope.nhom = [];
    $scope.history = [];
    $scope.filelist = [];
    ...
    $scope.fileall = function () {
        $http.get("/rest/files/taskfile").then(resp => {
        ...
        $http.get("/rest/files/tackfolder").then(resp => {
...
// Phân quyền hiẻn thị
    ...
    $scope.menu = "Giao việc tự động"
    $scope.quanly = "giaoviectudong"
    $scope.fileall();
})
;
