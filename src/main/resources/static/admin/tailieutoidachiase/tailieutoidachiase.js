app.controller("tailieutoidachiase-ctrl", function ($scope, $http) {
    $scope.fileshare = [];
    $scope.foldershare = [];
    ...
    $scope.fileall = function () {
        $http.get("/rest/shares/fileda").then(resp => {
            $scope.fileshare = resp.data;
            console.log($scope.fileshare)
        });
        $http.get("/rest/shares/folderda").then(resp => {
            $scope.foldershare = resp.data;
        });
    ...
    $scope.pager = {
        ...
        get fileshare() {
            ...
            return $scope.fileshare.slice(start, start + this.size);
        },
        get count() {
            return Math.ceil(1.0 * $scope.fileshare.length / this.size);
        },
        first() {
            this.page = 0;
        },
...
// Phân quyền hiẻn thị
    ...
    $scope.menu = "Tài liệu tôi đã chia sẻ"
    $scope.quanly = "tailieutoidachiase"
    $scope.fileall();
});
